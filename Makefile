...
recipes: ## List tracked evaluation recipes
	@$(ROBODOJO_BASE) catalog recipes --format table $(ARGS)
...
setup: _config-check ## Prepare XPolicyLab, locked env, assets, policy runtime, and checkpoint
	$(ROBODOJO_SETUP) workspace setup $(EXPERIMENT_SELECTION_ARGS) --seed "$(SEED)" --policy-gpu "$(POLICY_GPU)" $(ARGS)

preflight: _config-check ## Validate the configured experiment; DEEP=true checks policy readiness
	$(ROBODOJO_SIM) eval preflight $(EXPERIMENT_ARGS) $(DEEP_FLAG) $(ARGS)

eval: _eval-config-check ## Prepare and evaluate locally with built-in fast preflight
ifneq ($(strip $(DRY_RUN)),true)
	$(ROBODOJO_SETUP) workspace setup $(EXPERIMENT_SELECTION_ARGS) --seed "$(SEED)" --policy-gpu "$(POLICY_GPU)"
endif
	$(ROBODOJO_SIM) eval run $(EXPERIMENT_ARGS) --eval-num "$(EVAL_NUM)" $(EXPORT_SCENE_FLAG) $(PUBLISH_FLAG) $(DRY_RUN_FLAG) $(ARGS)
...
snapshots: _snapshot-config-check ## Capture first-frame RGB and optional USD bundles for selected or all recipes
	$(ROBODOJO_SIM) eval snapshots $(SNAPSHOT_ARGS) $(EXPORT_SCENE_FLAG) $(PUBLISH_FLAG) $(DRY_RUN_FLAG) $(ARGS)
...
smoke: _sweep-config-check ## Run one local episode for each selected recipe
	$(ROBODOJO_SIM) eval smoke $(SWEEP_ARGS) $(DRY_RUN_FLAG) $(ARGS)

benchmark: _sweep-config-check ## Run the configured local benchmark sweep
	$(ROBODOJO_SIM) eval benchmark $(SWEEP_ARGS) --eval-num "$(EVAL_NUM)" $(DRY_RUN_FLAG) $(ARGS)

##@ Inspection
tasks: ## List canonical tasks
	$(ROBODOJO_BASE) catalog tasks $(ARGS)

doctor: _config-check ## Inspect the configured simulator and policy adapter
	$(ROBODOJO_SIM) workspace doctor $(EXPERIMENT_SELECTION_ARGS) $(ARGS)
...
assets-moonlake-office: ## Build the pinned internal Moonlake office fixture
	$(ROBODOJO_SIM) workspace assets build-moonlake-office $(ARGS)

assets-moonlake-packing: assets-moonlake-office ## Build internal Moonlake packing task assets
	$(ROBODOJO_SIM) workspace assets build-moonlake-packing $(ARGS)
...
check: lint format-check test ## Run all non-mutating repository checks
	$(UV) run --locked robodojo --log-level "$(VERBOSITY)" catalog tasks --format json --check >/dev/null
	$(UV) run --locked robodojo --log-level "$(VERBOSITY)" catalog recipes --format json --check >/dev/null
