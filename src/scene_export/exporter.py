...
def _source_revisions(repo_root: Path) -> dict[str, Any]:
    result = {}
    for name, path in {
        "tracked_openarm_sources": repo_root / "scripts/assets/openarm_sources.json",
...
def _config_hashes(repo_root: Path, env) -> dict[str, str]:
    ...
    paths = [
        repo_root / "env_cfg" / f"{env.config_name}.yml",
        repo_root / "env_cfg/camera" / f"{eval_cfg['config']['camera']}.yml",
        repo_root / "env_cfg/scene" / f"{eval_cfg['config']['scene']}.yml",
        repo_root / "env_cfg/robot" / f"{eval_cfg['config']['robot']}.yml",
        repo_root / "env_cfg/sim" / f"{eval_cfg['config']['sim']}.yml",
