...
def local_scratch_root() -> Path:
    return _env_path("ROBODOJO_LOCAL_SCRATCH_ROOT") or REPO_ROOT / ".cache" / "robodojo-runtime"


def assets_root() -> Path:
    root = storage_root()
    return _env_path("ROBODOJO_ASSETS_ROOT") or (root / "assets" if root else REPO_ROOT / "Assets")
...
def data_root() -> Path:
    ...
    return (
        ...
        or (root / "datasets" if root else REPO_ROOT / "data")
    )


def model_root() -> Path:
    root = storage_root()
    return _env_path("ROBODOJO_MODEL_ROOT") or (
        root / "model_weights" if root else REPO_ROOT / "model_weights"
    )


def checkpoint_root() -> Path:
    root = storage_root()
    return _env_path("ROBODOJO_CHECKPOINT_ROOT") or (
        root / "model_weights" if root else REPO_ROOT / "checkpoints"
    )
...
def eval_root() -> Path:
    ...
    return _env_path("ROBODOJO_EVAL_ROOT") or (
        root / "runs" / "eval_result" / "RoboDojo" if root else REPO_ROOT / "eval_result" / "RoboDojo"
    )
...
def run_root() -> Path:
    root = storage_root()
    return _env_path("ROBODOJO_RUN_ROOT") or (root / "runs" if root else REPO_ROOT / "smoke_results")
